...
const DEV_SUBSTRATE_URL: &str = "wss://tfchain.dev.grid.tf/ws";
const DEV_GRID_PROXY_URL: &str = "https://gridproxy.dev.grid.tf";
const DEV_RELAY_URL: &str = "wss://relay.dev.grid.tf";
const RMB_SCHEMA: &str = "application/json";
const RMB_DEFAULT_TTL_SECS: u64 = 30;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct MyceliumData {
    #[serde(rename = "hex_key", with = "hex_bytes")]
    key: Vec<u8>,
...
struct MyceliumIpData {
    network: String,
    #[serde(rename = "hex_seed", with = "hex_bytes")]
    seed: Vec<u8>,
}
...
impl LiveClient {
    pub async fn devnet(mnemonic: &str) -> Result<Self, GridError> {
        ...
        let chain = OnlineClient::<PolkadotConfig>::from_url(DEV_SUBSTRATE_URL)
            .await
            .map_err(|err| GridError::backend(err.to_string()))?;
        let account_id: AccountId32 =
            <Keypair as subxt::tx::Signer<PolkadotConfig>>::account_id(&signer);
        let account_id = account_id.to_string();
        let twin_id = fetch_own_twin_id(&http, &account_id).await?;
        let client = Self {
            http,
            chain,
            signer,
            ...
            relay_url: DEV_RELAY_URL.to_string(),
            grid_proxy_url: DEV_GRID_PROXY_URL.to_string(),
        };
    ...
    async fn ensure_twin_relay(&self) -> Result<(), GridError> {
        let twin = fetch_twin(&self.http, self.identity.twin_id).await?;
    ...
    {
        let destination_twin = fetch_twin(&self.http, destination_twin_id).await?;
        let relay_hosts = vec![relay_host(&self.relay_url)?];
        let now = now_unix()?;
        let ttl = RMB_DEFAULT_TTL_SECS;
        let uid = random_uid();
        ...
        loop {
            ...
            let payload = match payload {
                ...
                Some(rmb_envelope::Payload::Cipher(bytes)) => {
                    ...
                    let source_twin = fetch_twin(&self.http, source_twin_id).await?;
...
fn build_network_light(name: &str, subnet: &str, mycelium_key: Vec<u8>) -> DeployWorkload {
    DeployWorkload {
        version: 0,
        name: name.to_string(),
...
fn sign_deployment(
    ...
    let signature = hex::encode(substrate_sign(signer, &challenge).as_ref());
...
fn deployment_hash_hex(deployment: &DeployDeployment) -> Result<String, GridError> {
...
async fn fetch_own_twin_id(http: &reqwest::Client, account_id: &str) -> Result<u32, GridError> {
    let response = http
        .get(format!("{DEV_GRID_PROXY_URL}/twins"))
...
async fn fetch_twin(http: &reqwest::Client, twin_id: u32) -> Result<ProxyTwin, GridError> {
    let response = http
        .get(format!("{DEV_GRID_PROXY_URL}/twins"))
...
fn empty_result_data() -> zos::ResultData {
    zos::ResultData {
        created: 0,
        state: String::new(),
        error: String::new(),
